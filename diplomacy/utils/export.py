...
def to_saved_game_format(game, output_path=None, output_mode='a'):
    ...
    for phase_dct in phases_to_dict:
        ...
        if phase_dct['name']]!='COMPLETED' :
            for m in game.message_history[phase_dct['name']].values():
                print(m)
#             phase_dct['messages'] = [m for m in game.message_history[phase_dct['name']].values()]
